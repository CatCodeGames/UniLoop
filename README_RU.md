...
|                   | Завершение (ms) | Скорость (%) | Alloc (KB) |
|-------------------|-----------------:|-------------:|----------------:|
| Coroutine         | 6.26             |122          | 0             |
| foreach в Update  | 7.64             |100          | 0             |
...
<details>
  <summary>Пояснение к тесту</summary> 
При использовании UniTask.WaitUntil отмена 2500 задач инициирует генерацию 2500 исключений OperationCanceledException. Это приводит к резкому выделению памяти и нагрузке на CPU из-за формирования Stack Trace для каждого исключения.
</details>

### Итоги
- Регистрация - по ресурсам CPU сопоставима с запуском Coroutine или UniTask. Затраты обусловлены подготовкой хэндлов и работой внутренних пулов.-  
- Выполнение - работает на скорости нативного цикла Update. Значительно превосходит корутины и UniTask при высокой нагрузке в игровом цикле.
- Завершение и отмена - отсутствие всплесков нагрузки. Предсказуемое поведение системы даже при одновременной остановке тысяч задач.


#### UniLoop vs UniTask (WaitUntil)

Базовая механика обоих инструментов идентична — выполнение логики в игровом цикле. Однако асинхронная операция по стандарту обязана выбросить исключение при отмене. При массовой остановке процессов это приводит к лавине исключений, падению FPS и скачкам аллокаций.
UniLoop реализует механизм отмены без исключений, что гарантирует нулевые аллокации при завершении и отмене задач.
